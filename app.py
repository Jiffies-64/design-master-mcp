            ...
            "tools": [
                {
                    ...
                    "description": "Start generating a design document",
                    "inputSchema": {
                        "type": "object",
                        "properties": {
                            "project_root_path": {
                        ...
                        "required": ["project_root_path", "user_id"]
                    }
                },
                {
                    "name": "get_next_step",
                    "description": "Get the next prompt in the sequence",
                    "inputSchema": {
                        "type": "object",
                        "properties": {
                            "template_id": {
                ...
                {
                    "name": "submit_placeholder_content",
                    "description": "Submit content for placeholders",
                    "inputSchema": {
                        "type": "object",
                        "properties": {
                            "placeholder_id": {
                        ...
                        "required": ["placeholder_id", "content"]
                    }
                },
                {
                    "name": "generate_complete_document",
                    "description": "Generate the complete design document",
                    "inputSchema": {
                        "type": "object",
                        "properties": {
                            "template_id": {
...
def mcp_tools():
    ...
    if method == 'start_document_generation':
        # Simulate the request for the tool function
        request.json = params
        return start_document_generation()
    elif method == 'get_next_step':
        # Simulate the request for the tool function
        request.args = params
        return get_next_step()
    elif method == 'submit_placeholder_content':
        # Simulate the request for the tool function
        request.json = params
        return submit_placeholder_content()
    elif method == 'generate_complete_document':
        # Simulate the request for the tool function
        request.json = params
        return generate_complete_document()
    else:
...
def mcp_sse():
    ...
    def generate():
        yield f"data: {json.dumps({'status': 'connected', 'message': 'MCP SSE connection established'})}\n\n"
        
        # Keep connection alive
        while True:
            yield f"data: {json.dumps({'status': 'alive', 'timestamp': str(int(time.time()))})}\n\n"
